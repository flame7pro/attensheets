...
class DeviceUpdateRequest(BaseModel):
    """Request model for updating student device"""
    student_email: str
    new_device_id: str
    new_device_info: Dict[str, Any]
    reason: Optional[str] = "Device change requested by administrator"

class DeviceRemovalRequest(BaseModel):
    """Request model for removing a device"""
    student_email: str
    device_id: str

class DeviceListRequest(BaseModel):
    """Request model for listing devices"""
    student_email: str
...
def hash_device_id(device_id: str) -> str:
    """
    Hash device ID for secure storage (similar to password hashing)
    Uses SHA-256 for one-way encryption
    """
    return hashlib.sha256(device_id.encode()).hexdigest()

def verify_device_id(plain_device_id: str, hashed_device_id: str) -> bool:
    """Verify a device ID against its hash"""
    return hash_device_id(plain_device_id) == hashed_device_id
...
@app.get("/admin/student-devices/{student_email}")
async def list_student_devices(student_email: str, email: str = Depends(verify_token)):
    """
    List all trusted devices for a student (Admin/Teacher only)
    Returns device info without exposing raw device IDs
    """
    try:
        # Verify requester is a teacher/admin
        requester = db.get_user_by_email(email)
        if not requester:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only teachers/administrators can manage student devices"
            )
        ...
        # Return sanitized device list (no raw device IDs)
        sanitized_devices = []
        for device in trusted_devices:
            sanitized_devices.append({
                "device_hash": device.get("id", "")[:16] + "...",  # Show only first 16 chars
                "name": device.get("name", "Unknown Device"),
                "browser": device.get("browser", "Unknown"),
                "os": device.get("os", "Unknown"),
                "device_type": device.get("device", "Unknown"),
                "first_seen": device.get("first_seen", "Unknown"),
                "last_seen": device.get("last_seen", "Unknown"),
                "login_count": device.get("login_count", 0)
            })
        ...
        print(f"Error listing student devices: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to list student devices"
        )

@app.post("/admin/student-devices/add")
async def add_student_device(request: DeviceUpdateRequest, email: str = Depends(verify_token)):
    """
    Add a new trusted device for a student (Admin/Teacher only)
    This allows students to login from a new device after admin approval
    """
    try:
        # Verify requester is a teacher/admin
        requester = db.get_user_by_email(email)
        ...
        # Get student data
        student = db.get_student_by_email(request.student_email)
        if not student:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Student not found"
        ...
        hashed_device_id = hash_device_id(request.new_device_id)
        
        # Prepare new device data
        new_device = {
            "id": hashed_device_id,  # ✅ Store hashed version
            "raw_id": request.new_device_id,  # ✅ Store raw for admin management (encrypted in DB)
            "name": request.new_device_info.get("name", "Unknown Device"),
            "browser": request.new_device_info.get("browser", "Unknown"),
            "os": request.new_device_info.get("os", "Unknown"),
            "device": request.new_device_info.get("device", "Unknown"),
            "first_seen": datetime.now(timezone.utc).isoformat(),
            "last_seen": datetime.now(timezone.utc).isoformat(),
            "login_count": 0,
            "added_by": email,  # Track who added this device
        ...
        # Check if device already exists (by hash)
        device_exists = any(d.get("id") == hashed_device_id for d in trusted_devices)
        
        if device_exists:
            return {
                "success": False,
                "message": "This device is already trusted for this student"
            }
        ...
        db.update_student(student["id"], {"trusted_devices": trusted_devices})
        
        return {
            "success": True,
            "message": f"Device added successfully for {student.get('name')}",
            "device_info": {
        ...
        print(f"Error adding student device: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to add student device"
        )

@app.delete("/admin/student-devices/remove")
async def remove_student_device(request: DeviceRemovalRequest, email: str = Depends(verify_token)):
    """
    Remove a trusted device from a student's account (Admin/Teacher only)
    Useful when a device is lost/stolen or student requests removal
    """
    try:
        # Verify requester is a teacher/admin
        requester = db.get_user_by_email(email)
        ...
        # Hash the device ID to find it
        hashed_device_id = hash_device_id(request.device_id)
        ...
        for device in trusted_devices[:]:  # Use slice to avoid modification during iteration
            if device.get("id") == hashed_device_id or device.get("raw_id") == request.device_id:
                removed_device_name = device.get("name", "Unknown Device")
                trusted_devices.remove(device)
                device_found = True
                break
        ...
        if not device_found:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Device not found for this student"
            )
        
        # Ensure at least one device remains (security measure)
        if len(trusted_devices) == 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Cannot remove last trusted device. Student must have at least one trusted device."
            )
        ...
        return {
            "success": True,
            "message": f"Device '{removed_device_name}' removed successfully",
            "remaining_devices": len(trusted_devices)
        }
        
    except HTTPException:
        raise
    except Exception as e:
        print(f"Error removing student device: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to remove student device"
        )

@app.put("/admin/student-devices/replace")
async def replace_student_device(
    old_device_id: str,
    request: DeviceUpdateRequest,
    email: str = Depends(verify_token)
):
    """
    Replace an old device with a new one (Admin/Teacher only)
    Useful when student gets a new phone/laptop
    """
    try:
        # Verify requester is a teacher/admin
        requester = db.get_user_by_email(email)
        ...
        # Hash device IDs
        old_hashed = hash_device_id(old_device_id)
        new_hashed = hash_device_id(request.new_device_id)
        ...
        # Find old device
        device_found = False
        for idx, device in enumerate(trusted_devices):
            if device.get("id") == old_hashed or device.get("raw_id") == old_device_id:
                # Replace with new device
                trusted_devices[idx] = {
                    "id": new_hashed,
                    "raw_id": request.new_device_id,
                    "name": request.new_device_info.get("name", "Unknown Device"),
                    "browser": request.new_device_info.get("browser", "Unknown"),
                    "os": request.new_device_info.get("os", "Unknown"),
                    "device": request.new_device_info.get("device", "Unknown"),
        ...
        if not device_found:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Old device not found for this student"
            )
        ...
        return {
            "success": True,
            "message": f"Device replaced successfully for {student.get('name')}",
            "new_device_info": {
                "name": request.new_device_info.get("name"),
                "browser": request.new_device_info.get("browser"),
                "os": request.new_device_info.get("os")
            }
        }
        ...
        print(f"Error replacing student device: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to replace student device"
        )

@app.post("/admin/student-devices/reset-all")
async def reset_all_student_devices(student_email: str, email: str = Depends(verify_token)):
    """
    Reset all trusted devices for a student (Emergency use only)
    Student will need to re-register their device on next login
    """
    try:
        # Verify requester is a teacher/admin
        requester = db.get_user_by_email(email)
        ...
        # Clear all devices
        db.update_student(student["id"], {"trusted_devices": []})
        
        return {
            "success": True,
            "message": f"All devices cleared for {student.get('name')}. Student will need to re-register on next login.",
            "student_email": student_email
        }
        ...
        print(f"Error resetting student devices: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to reset student devices"
        )

# ==================== STUDENT SELF-SERVICE DEVICE MANAGEMENT ====================

@app.get("/student/my-devices")
async def get_my_devices(email: str = Depends(verify_token)):
    """
    Allow students to view their own trusted devices (read-only)
    """
    try:
        student = db.get_student_by_email(email)
        if not student:
        ...
        return {
            ...
            "devices": sanitized_devices,
            "device_count": len(trusted_devices),
            "message": "To add or remove devices, please contact your teacher or administrator"
        }
